...
short* conv(short *a, short *b, short an, short bn)
{
	...
	rev(a, an);
	short* bnew;
	bnew = (short *)malloc((an + an - 1 + bn - 1) << 1);
	short i = 0;
...
void main()
{
	...
	{
		inputs[i] = (short)(input[i] * 0.78125 * (1 << 7));
		printf("%d\t", inputs[i]);
		i++;
	}
	printf("\n");
	...
	{
		filters[i] = (short)(filter[i] * 0.78125 * (1 << 7));
		printf("%d\t", filters[i]);
		i++;
	}
	printf("\n");
